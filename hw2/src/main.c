...
option_table[] = {
 ...
 {OUTPUT,         "output",    'o',      required_argument, "address",
                  "Print the output to specified file"}
};
...
char *argv[];
{
        ...
        while(optind < argc) {
            if((optval = getopt_long(argc, argv, short_options, long_options, NULL)) != -1) {
                switch(optval) {
                case REPORT: case 'r': report++; break;
                case COLLATE: case 'c': collate++; break;
                case OUTPUT: case 'o':
                        if(optarg){
                            fp1=freopen(optarg, "w+",stdout);
                    }
                        else{
                            fprintf(stderr,"Option '%s' requires argument of output file name.\n\n",
                                option_table[(int)optval].name);
                            usage(argv[0]);
                        }
                    break;
                case TABSEP: tabsep++; break;
                case NONAMES: case 'n': nonames++; break;
                case SORTBY: case 'k':
