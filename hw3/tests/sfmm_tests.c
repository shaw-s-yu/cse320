...
Test(sf_memsuite_student, free_pre_allocated_0, .init = sf_mem_init, .fini = sf_mem_fini){
	void* y=sf_malloc(sizeof(int));
	void* z=sf_malloc(sizeof(int));

	sf_free(y);

	sf_header* hp=(sf_header*)(z-8);
	cr_assert(hp->info.prev_allocated==0, "pre_allocated bit is not set after free");
}

Test(sf_memsuite_student, last_block_size_of_page, .init = sf_mem_init, .fini = sf_mem_fini){
	void* x=sf_malloc(12088);
	sf_header* hp=(sf_header*)((void*)x-8);

	cr_assert(hp->info.block_size<<4==12240, "Wrong malloc block size if last free block size is less than 32");

}

Test(sf_memsuite_student, malloc_pre_allocated_1, .init = sf_mem_init, .fini = sf_mem_fini){
	void* y=sf_malloc(sizeof(int));

	sf_header* hp=(sf_header*)((void*)y-8);
	hp=(void*)hp+(hp->info.block_size<<4);
	cr_assert(hp->info.prev_allocated==1, "pre_allocated bit is not set after malloc");
}
...
Test(sf_memsuite_student, realloc_size_colease_free_blocks, .init = sf_mem_init, .fini = sf_mem_fini){
	...
	cr_assert(sf_errno!=EINVAL, "errno is set to EINVAL");
	cr_assert(sf_errno!=ENOMEM, "errno is set to ENOMEM");
